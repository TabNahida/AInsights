...
class DocsMarkupTests(unittest.TestCase):
    ...
    def test_custom_weights_show_metric_coverage_and_scatter_leaders(self):
        ...
        self.assertIn('metrics: ["AA-LCR", "AA-Omniscience Accuracy"]', app_js)
        self.assertIn('metrics: ["Humanity\'s Last Exam", "GPQA Diamond", "CritPt"]', app_js)
        self.assertNotIn('metrics: ["GDPval-AA", "τ²-Bench Telecom", "Terminal-Bench Hard"]', app_js)
        self.assertNotIn('metrics: ["AA-Omniscience Accuracy", "AA-Omniscience Non-Hallucination Rate"]', app_js)
